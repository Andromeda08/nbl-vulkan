...
# ======================================= #
g_shader_langs = ["glsl"]                                                   # Per shader language directories for shaders
g_bin_dir = "bin"                                                           # Default output directory
g_target_vulkan_env = "vulkan1.4"                                           # Default target Vulkan environment
    ...
    for lang in g_shader_langs:
        ...
        for shader in shaders:
            ...
            for directory in g_excluded_dirs:
                if f"{directory}{g_sep}" in shader:
                    is_excluded = True
            if not is_excluded:
...
def get_output_shader_name(shader, options: Options):
    name = f"{g_bin_dir}{g_sep}{shader.rsplit(g_sep)[-1]}.spv"
...
def compile_hlsl(shader: str, options: Options):
    command = f"dxc -spirv -T {get_hlsl_profile(shader)} -E main -fspv-target-env={options.target_env} {shader} -Fo {get_output_shader_name(shader, options)}"
    with subprocess.Popen(command, stdout=subprocess.PIPE, shell=True) as proc:
        if not options.silent:
            print(f"- {shader.rsplit(g_sep)[-1]}")
